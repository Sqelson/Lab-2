...
private:
    ...
    void findMin(Node* node, Node*& parent, Node*& minNode) {
        ...
        while (minNode->left != nullptr) {  // Traverse to the leftmost node in the right subtree
    ...
    void removeNodeWorker(Node*& node, KeyType key) {
        ...
        if (key == node->data) {  // If the key matches the current node's data
            ...
            // Case 2: Node with two children
                ...
                if (parentMinNode->left == minNode) {
                    removeNodeWorker(parentMinNode->left, minNode->data);  // Recursively remove the in-order successor in the left subtree
                }
                else {
                    removeNodeWorker(parentMinNode->right, minNode->data);  // Recursively remove the in-order successor in the right subtree
                }
            }
        }
        else if (key < node->data) {  // If the key is less than the current node's data
            removeNodeWorker(node->left, key);  // Recursively search for the key in the left subtree
        }
        else {  // If the key is greater than the current node's data
